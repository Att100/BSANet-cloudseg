...
from models.loss import _bce_iou_loss_with_aux
...
def bce_loss_with_aux(pred, target, weight=[1, 0.6, 0.4, 0.2]):
    return _bce_iou_loss_with_aux(pred, paddle.unsqueeze(target, 1), weight)
...
def train(args):
    ...
    paddle.save(
        model.state_dict(), 
        "./ckpts/{}-iou_epochs_{}.pdparam".format(args.model_tag, args.epochs))
